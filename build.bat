...
REM
REM  Usage:
REM    build.bat           one-file GUI exe (dist\CrapCleaner.exe)
REM    build.bat onedir    folder build, faster startup
...
set "DATAFLAG=--add-data "crapcleaner/gui/assets;crapcleaner/gui/assets""

echo [build] Building %MODE% executable...
if "%MODE%"=="onefile" (
    %PY% -m PyInstaller --noconfirm --clean --onefile --windowed --name CrapCleaner %DATAFLAG% build_launcher.py
) else (
    %PY% -m PyInstaller --noconfirm --clean --onedir --windowed --name CrapCleaner %DATAFLAG% build_launcher.py
)
if errorlevel 1 (
